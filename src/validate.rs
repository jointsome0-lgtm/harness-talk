...
pub fn uuid(value: &str) -> Result<String, Error> {
    uuid::Uuid::parse_str(value).map(|id| id.to_string())
        .map_err(|_| Error::code("badly formed hexadecimal UUID string"))
}
pub fn peer_name(value: &str) -> Result<(), Error> {
    let b = value.as_bytes();
    if b.is_empty() || b.len() > 64 || !b[0].is_ascii_lowercase() && !b[0].is_ascii_digit()
...
pub fn opencode_session_id(value: &str) -> Result<String, Error> {
    if !value.starts_with("ses") || value.len() < 4 || value.len() > 256
...
pub fn opencode_url(value: Option<&str>) -> Result<String, Error> {
    let value = value.unwrap_or("http://127.0.0.1:4096");
    ...
    if !matches!(url.scheme(), "http" | "https") || host.is_empty() || authority.contains('@')
        || url.query().is_some_and(|q| !q.is_empty()) || url.fragment().is_some_and(|f| !f.is_empty()) {
        return Err(invalid());
    }
    if !local { return Err(Error::code("opencode_url_must_be_loopback")); }
    let rest = rest.split(['?', '#']).next().unwrap_or(rest).trim_end_matches('/');
    Ok(format!("{}://{}", url.scheme(), rest))
}
...
pub fn page(limit: i64, cursor: Option<i64>) -> Result<(), Error> {
    if !(1..=500).contains(&limit) { return Err(Error::code("limit_must_be_between_1_and_500")); }
    if cursor.is_some_and(|c| c < 1) { return Err(Error::code("seq_cursor_must_be_a_positive_integer")); }
    Ok(())
}
pub fn python_whitespace(c: char) -> bool { c.is_whitespace() || matches!(c, '\u{1c}'..='\u{1f}') }
pub fn preview(body: &str) -> String {
    body.split(['\n', '\r', '\u{b}', '\u{c}', '\u{1c}', '\u{1d}', '\u{1e}', '\u{85}', '\u{2028}', '\u{2029}'])
        .map(|line| line.trim_matches(python_whitespace)).find(|line| !line.is_empty())
