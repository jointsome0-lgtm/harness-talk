...
pub fn notification(peer:&Peer, message:&Message, database:&Path) -> String {
    ...
    let command=os::shell_join(&["htalk", "--db", &db, "--as", &peer.name, "show", &message.row.id]);
    format!("[harness-talk peer notification; message {}]\n\
        ...
        Reading does not acknowledge the message. Reply and ack through htalk when appropriate.", message.row.id, command)
}
...
pub fn probe(peer:&Peer) -> Result<Value,Failure> {
    match peer.harness { Harness::Claude => crate::claude::probe(peer), Harness::Codex => crate::codex::probe(peer), Harness::Opencode => crate::opencode::probe(peer) }
}
