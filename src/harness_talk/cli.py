...
def parser():
    ...
    send.add_argument("--wait", type=float, default=0, metavar="SECONDS",
                      help="Wait 0–45 seconds for an answer (default: %(default)s). An answer recorded by this wait "
                           "before the final notification check skips that notice. It does not resend.")
    wait = commands.add_parser("wait", help="Wait for an answer to a saved request.",
                               description="Poll a saved outgoing request for its answer. Record the answer before returning it; "
                                           "a notification that sees this record at its final check is skipped. An already accepted notice may still arrive. "
                                           "Safe to resume after timeout or interruption; does not resend or acknowledge.",
