...
A message acknowledged before the notification claim is not submitted. Claude and OpenCode also recheck the saved acknowledgment after client preflight, immediately before sending. An acknowledgment received during that preflight prevents transmission and records `not_submitted` with `acknowledged_before_notification`. The message and read mark remain saved; the one-attempt rule still applies. A race after this final check, including a notice already accepted by the client, remains possible. This check cannot recall such a notice or guarantee that it never causes another model turn.
