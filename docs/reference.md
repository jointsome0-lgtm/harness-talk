...
| Code | Meaning |
| --- | --- |
| 0 | Local operation succeeded, including `--no-notify`, an identical retry, acknowledgment before notification completed, an answer already returned by the requester's wait, a notice skipped for a retired recipient, or `send --wait` that returned a saved answer. |
